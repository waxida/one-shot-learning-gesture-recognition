        ...
        if return_last:
            ...
            if return_seq_2d:
                ...
                if self.inputs.get_shape().ndims==3: #Guangming Zhu
                    ...
                    self.outputs = tf.reshape(tf.concat(outputs,1), [-1, height, width, n_hidden]) #Guangming Zhu
            else:
                ...
                if self.inputs.get_shape().ndims==3: #Guangming Zhu
                    ...
                    self.outputs = tf.reshape(tf.concat(outputs,1), [-1, n_steps, height, width, n_hidden]) #Guangming Zhu
            ...
            if return_last:
                ...
                if return_seq_2d:
                    ...
                    if self.inputs.get_shape().ndims==3: #Guangming Zhu
                        ...
                        self.outputs = tf.reshape(tf.concat(outputs,1), [-1, height, width, n_hidden*2]) #Guangming Zhu
                else:
                    # <akara>: stack more RNN layer after that
                    # 3D Tensor [n_example/n_steps, n_steps, n_hidden]
                    if self.inputs.get_shape().ndims==3: #Guangming Zhu
                        ...
                        self.outputs = tf.reshape(tf.concat(outputs,1), [-1, n_steps, height, width, n_hidden*2])
    ...
    def __init__(
        ...
        if return_last:
            ...
            if return_seq_2d:
                # PTB tutorial:
                # 2D Tensor [n_example, n_hidden]
                self.outputs = tf.reshape(tf.concat(output_fw,1 ), [-1, n_hidden])
            else:
                # <akara>:
                # 3D Tensor [n_example/n_steps, n_steps, n_hidden]
                self.outputs = tf.reshape(tf.concat(output_fw,1), [-1, n_steps, n_hidden])
